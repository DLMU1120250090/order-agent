...
class ClarifyRuleService:
    ...
    def missing_slots(self, slots: TravelSlotBundle, fuzzy_date: bool = False) -> List[str]:
        ...
        # 3. 乘车人员/人数（出行必问）
        if not safe_slots.passengers and not safe_slots.companion:
            missing.append("passengers")

        # 4. 预算条件必填：无预算且无其它强偏好时追问
    ...
    def has_strong_preference(self, slots: TravelSlotBundle) -> bool:
        return bool(slots.travelStyle or slots.transportMode or slots.companion or slots.passengers)
    ...
    def fallback_question(self, missing_slots: List[str]) -> str:
        ...
        if "passengers" in missing_slots:
            return "这次出行一共有几位？是您本人出行，还是有其他同行朋友？"
